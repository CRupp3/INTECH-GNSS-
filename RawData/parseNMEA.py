    ...
    if ports and port_index < len(ports):
        ...
        try:
            ...
            s = serial.Serial(
                ...
                stopbits=serial.STOPBITS_ONE,  # Fixed stop bits
                bytesize=serial.EIGHTBITS  # Fixed byte size
            )
            ...
            print(f"Error opening the serial port {port}: {e}")
    else:
        print("No serial ports found or invalid index.")
    return None
    
def reset_serial_connection(s, retry=3):
    ...
    print("Failed to reset serial port after several attempts.")
    return None
        ...
        if len(buffer) > max_buffer_size:
            ...
            if buffer_exceeded_count >= 1:
                ...
                if serial_port is None:
                    ...
                    raise Exception("Failed to reset serial port")
                buffer_exceeded_count = 0  # Reset the counter after attempting a reset
